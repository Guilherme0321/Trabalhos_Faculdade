...
public class LinearSearch {
    ...
    public static boolean linearSearch(String nome){
        boolean isFound = false;
        for(int i = 0; i < playes.length; i++){
            if(Equals(playes[i].getNome(), nome)){
                isFound = true;
                i = playes.length;
            }
        }
    ...
    public static void main(String[] args) {
        String entrada = "";
        playes = new Jogador[0];
        Jogador[] jogadores = Jogador.ler();
        while(!entrada.equals("FIM")){
            entrada = MyIO.readLine();
            if(!entrada.equals("FIM")){
                playes = Jogador.add(playes, jogadores[Integer.parseInt(entrada)]);
            }
        }
        entrada = "";
        while(!entrada.equals("FIM")){
            entrada = MyIO.readLine();
            if(!entrada.equals("FIM")){
                boolean found = linearSearch(entrada);
                if(found){
